...
import (
	...
	models2 "tiktok/source/apps/usersrv/models"
)
...
func RegisterHandler(ctx context.Context, p *io.ParamRegister) error {
	...
	// 1.判断用户存不存在
	flag, err := models2.CheckUserExist(ctx, p.Username)
	if err != nil {
	...
	// 3.保存进数据库
	return models2.InsertOneUser(ctx, user)
}

func Login(p *io.ParamLogin) (userId int64, token string, err error) {

	// 调用登录操作
	if userId, err = models2.Login(p.Username, p.Password); err != nil {
		return -1, "", err
	}
...
func GetUserInfo(ctx *gin.Context, p *io.UserInfoReq, claim *jwt.MyClaims) (*io.User, error) {
	...
	// 查询用户信息
	user, err := models2.FindOneUser(ctx, userInfo)
	if err != nil {
		return nil, err
	}
	...
	// 获取用户的粉丝数
	fansCount, err := models2.CountUserFans(ctx, p.UserID)
	if err != nil {
		return nil, err
	}
	userResp.FollowerCount = fansCount

	// 获取用户关注数
	followCount, err := models2.CountUserStar(ctx, p.UserID)
	if err != nil {
		return nil, err
	}
	...
	// 获取是否已经关注
	userResp.IsFollow, err = models2.IsFans(ctx, claim.UserID, p.UserID)
	if err != nil {
		return nil, err
	}
	return userResp, nil
